<div id="$HolderID" class="form-check field<% if $extraClass %> $extraClass<% end_if %><% if $Required %> required<% end_if %><% if $MessageType %><% include SilverWare\Forms\State\Holder Type=$MessageType %><% end_if %>">
